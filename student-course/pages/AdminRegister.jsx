...
const AdminRegister = () => {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    const token = localStorage.getItem("token"); // Ensure only logged-in admins can register new admins
    ...
    try {
      const response = await fetch("http://localhost:5000/api/admin/register", {
        ...
        headers: {
          ...
          Authorization: `Bearer ${token}`, // Ensure only admins can access
        },
      ...
      if (response.ok) {
        toast({ title: "Admin Registered Successfully!", status: "success", duration: 3000 });
        navigate("/admin");
      } else {
  ...
  return (
    <Flex justify="center" align="center" height="100vh" bg="gray.100">
      <Box bg="white" p={8} borderRadius="lg" boxShadow="lg" width="400px">
        <Heading textAlign="center" mb={6}>Admin Registration</Heading>
        <form onSubmit={handleRegister}>
          <VStack spacing={4}>
            ...
            <Button type="submit" colorScheme="blue" width="full">Register Admin</Button>
          </VStack>
        </form>
      </Box>
    </Flex>
